export const PADDING_MEDIUM = 10;
export const PADDING_MEDIUM_1 = 20;

export const MARGIN_SMALL = 5;
export const MARGIN_MEDIUM = 15;
export const MARGIN_MEDIUM_1 = 20;
export const MARGIN_LARGE = 60;

export const FONT_SIZE_MEDIUM = 15;
export const FONT_SIZE_MEDIUM_LARGE = 20;
export const FONT_SIZE_MEDIUM_LARGE_1 = 25;
export const FONT_SIZE_LARGE = 35;

export const FONT_SIZE_ICON_MEDIUM = 40;

export const FONT_WEIGHT = '400';
export const FONT_WEIGHT_LARGE = '700';

export const BODER_RADIUS_SMALL_1 = 4;
export const BODER_RADIUS_SMALL_2 = 6;
export const BODER_MEDIUM = 30;

export const HEIGHT_SMALL_MEDIUM = 50;

export const WIDTH_SMALL_1 = 30;
export const HEIGHT_SMALL_1 = 30;
export const HEIGHT_SMALL_2 = 50;
export const WIDTH_SMALL_3 = 60;
export const HEIGHT_SMALL_3 = 60;

export const WIDTH_MEDIUM = 300;
export const HEIGHT_MEDIUM = 250;

